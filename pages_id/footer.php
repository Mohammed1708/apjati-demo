<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Document</title>
  <link rel="stylesheet" href="../dist/output.css" />
  <link rel="stylesheet" href="../src/input.css" />

  <!-- CSS -->
  <link
    rel="stylesheet"
    href="../node_modules/@splidejs/splide/dist/css/splide.min.css" />

  <!-- JavaScript -->
  <script src="../node_modules/"></script>
  <!-- <script src="../node_modules/@splidejs/splide/dist/js/splide.min.js"></script> -->

  <!-- font google -->
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin />
  <link
    href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap"
    rel="stylesheet" />
</head>

<body class="list-none">
  <footer class="text-black mt-20" style="background-color:  #CBA344;">
    <div class="container mx-auto px-6 py-8 flex flex-col justify-center">
      <!-- Bagian Atas Footer -->
      <div class="flex flex-col lg:flex-row justify-center items-start gap-6">
        <!-- Informasi APJATI -->
        <div class="w-full md:w-1/3">
          <h1 class="font-semibold text-xl mb-4">KANTOR APJATI</h1>
          <div class="space-y-4">
            <p class="flex items-center text-sm gap-4">
              <img src="../src/assets/icon/mynaui--location.svg" alt="Location" class="w-6 h-6" />
              Jl. Buncit Raya No.126, RT.10/RW.2, Duren Tiga, Pancoran, Kota Jakarta Selatan, Daerah Khusus Ibukota Jakarta 12790, Indonesia
            </p>
            <p class="flex items-center text-sm gap-4">
              <img src="../src/assets/icon/material-symbols--call-outline.svg" alt="Phone" class="w-6 h-6" />
              (021) 7994001
            </p>
            <p class="flex items-center text-sm gap-4">
              <img src="../src/assets/icon/ic--outline-email.svg" alt="Email" class="w-6 h-6" />
              infoapjati@apjati.org.id
            </p>
          </div>
        </div>

        <!-- Menu dan Media Sosial -->

        <div class="flex gap-3 lg:gap-5">
          <!-- Menu -->
          <div class="">
            <h1 class="font-semibold text-xl mb-4">Menu</h1>
            <ul class="space-y-2 text-sm">
              <li><a href="index-id.html" class="hover:underline">Beranda</a></li>
              <li><a href="daftar_apjapti.php" class="hover:underline">Anggota APJATI</a></li>
              <li><a href="regulasi.php" class="hover:underline">Regulasi</a></li>
              <li><a href="about_us.php" class="hover:underline">Tentang Kami</a></li>
              <li><a href="contact_us.php" class="hover:underline">Kontak Kami</a></li>
            </ul>
          </div>
          <!-- anggota apjati -->
          <div class="">
            <h1 class="font-semibold text-xl mb-4">Anggota</h1>
            <ul class="space-y-2 text-sm">
              <li><a href="list_anggota.php" class="hover:underline">Anggota Kami</a></li>
              <li><a href="list_anggota.php" class="hover:underline">Daftar Jadi Anggota</a></li>
            </ul>
          </div>
        </div>
        <div class="flex gap-3 lg:gap-5">
          <!-- Tentang Kami -->
          <div class="">
            <h1 class="font-semibold text-xl mb-4">Tentang Kami</h1>
            <ul class="space-y-2 text-sm">
              <li><a href="about_us.php" class="hover:underline">Tentang APJATI</a></li>
              <li>
                <a href="department.php" class="hover:underline">Struktur APJATI</a>
              </li>
            </ul>
          </div>
        </div>
        <!-- Media Sosial -->
        <div class="">
          <h1 class="font-semibold text-xl mb-4">Social Media</h1>
          <ul class="space-y-2 text-sm">
            <li><a href="#" class="hover:underline">Tiktok</a></li>
            <li><a href="https://www.instagram.com/apjati.id?igsh=OGJqYW0yaGJuZ3J2" class="hover:underline">Instagram</a></li>
            <li><a href="#" class="hover:underline">Facebook</a></li>
            <li><a href="#" class="hover:underline">Twitter</a></li>
            <li><a href="#" class="hover:underline">YouTube</a></li>
          </ul>
        </div>
      </div>

      <!-- Garis Pemisah -->
      <hr class="my-6 border-black" />

      <!-- Bagian Footer Bawah -->
      <div class="flex flex-col sm:flex-row justify-between items-center text-sm text-black">
        <span>© 2023
          <a href="https://flowbite.com/" class="hover:underline">APJATI™</a>.
          All Rights Reserved.</span>
        <div class="mt-4 sm:mt-0">
          <!-- Tambahkan ikon sosial media di sini jika ada -->
        </div>
      </div>
    </div>
  </footer>